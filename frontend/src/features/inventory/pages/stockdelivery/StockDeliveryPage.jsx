...
const DELIVERY_EXPORT_COLUMNS = [
  ...
  { key: 'status', label: 'Trạng thái', width: 16, defaultChecked: true, getValue: (delivery) => delivery.status === 'DRAFT' ? 'Lưu tạm' : getStatusLabel(delivery.status) },
...
const DELIVERY_DETAIL_EXPORT_COLUMNS = [
  ...
  { key: 'status', label: 'Trạng thái', width: 16, defaultChecked: false, getValue: (row) => row.status === 'DRAFT' ? 'Lưu tạm' : getStatusLabel(row.status) },
