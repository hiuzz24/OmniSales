...
test.describe('Notification E2E Tests', () => {
  ...
  test('NOTI-E2E-1 - Notification page loads and renders list', async ({ managerPage }) => {
    await gotoOrSkip(managerPage, '/notifications', { waitUntil: 'domcontentloaded' });
  });

  // NOTI-E2E-2 - bell icon is on the dashboard, independent of /notifications route
  test('NOTI-E2E-2 - Bell icon in header opens notification panel', async ({ managerPage }) => {
  ...
  test('NOTI-E2E-3 - Filter "Chưa đọc" only shows unread notifications', async ({ managerPage }) => {
    await gotoOrSkip(managerPage, '/notifications', { waitUntil: 'domcontentloaded' });
  });

  // NOTI-E2E-4 - skipped: depends on /notifications page
  test('NOTI-E2E-4 - Click "Đánh dấu đã đọc" marks single notification', async ({ managerPage }) => {
    await gotoOrSkip(managerPage, '/notifications', { waitUntil: 'domcontentloaded' });
  });

  // NOTI-E2E-5 - skipped: depends on /notifications page
  test('NOTI-E2E-5 - Click "Đánh dấu tất cả đã đọc" marks all notifications', async ({ managerPage }) => {
    await gotoOrSkip(managerPage, '/notifications', { waitUntil: 'domcontentloaded' });
  });
});
