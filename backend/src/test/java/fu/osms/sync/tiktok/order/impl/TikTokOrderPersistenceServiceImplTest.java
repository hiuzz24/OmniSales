...
class TikTokOrderPersistenceServiceImplTest {
    ...
    void setUp() {
        service = new TikTokOrderPersistenceServiceImpl(
                ...
                channelVariantRepository, metadataMapper);
    }
    ...
    void write_created() {
        ...
        when(metadataMapper.merge(any(), any(), any(), org.mockito.ArgumentMatchers.anyBoolean())).thenReturn(new java.util.HashMap<>());
    ...
    void write_resolvesVariant() {
        ...
        verify(channelVariantRepository)
                .findActiveByChannelIdAndExternalVariantId(eq(channel.getId()), eq("ext-var-1"));
    }
