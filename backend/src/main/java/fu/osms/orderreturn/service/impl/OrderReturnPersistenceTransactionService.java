...
public class OrderReturnPersistenceTransactionService {
    ...
    public Optional<UUID> upsertOnce(UUID channelId, OrderReturnSnapshot snapshot) {
        ...
        if (platformProcessApplied
                ...
                && refundCoversReceivedQuantities(mapping.items())) {
    ...
    private boolean refundCoversReceivedQuantities(List<OrderReturnItem> items) {
        List<OrderReturnItem> receivedItems = items.stream()
                .filter(item -> item.getReceivedQuantity() != null && item.getReceivedQuantity() > 0)
                .toList();
        return !receivedItems.isEmpty() && receivedItems.stream().allMatch(item ->
